...
\section{Languages}
  ...
  For example, take the set A = \{ 0, 1 \} as an alphabet.
...
\section{Turing machines}
  ...
  They are simple machines, consisting of a single read/write head and an infinite tape of cells.
  ...
  Each Turing machine is defined by a set of rules.
...
\end{description}
  ...
  They form a string over the alphabet $\Sigma$.
...
\section{State tables}
  A Turing machine can be summarised in a five-columned table called a state table.
...
\end{figure}
  ...
  We usually define the complexity class \CC{NTIME}\((f(n)) \) as the set of all languages for which there is a decider that takes \( O\left(f(n)\right) \) steps.
